...
export default function UploadMaterial() {
  ...
  function handleFileSelect(selectedFile) {
    ...
    if (selectedFile.size > MAX_FILE_SIZE_BYTES) {
      ...
      setError(`File size (${sizeMB} MB) exceeds the 15 MB limit for direct upload. For video lectures or large files, please select the "External Link" option to paste a YouTube, Vimeo, or Google Drive link.`)
      setFile(null)
      return
    }
  ...
  return (
    <AppLayout role="lecturer">
      <nav className="flex items-center gap-2 text-[12px] font-bold text-[#44474f] mb-6">
        ...
        <span className="text-[#03224d]">{course?.title ?? 'Course'} — Upload Material</span>
      </nav>
      ...
      <div className="max-w-2xl">
        ...
        {done ? (
          <div className="bg-[#a0f3d4] border border-[#086b53] rounded-xl p-8 text-center">
            <span className="material-symbols-outlined text-[48px] text-[#086b53] block mb-3" style={{ fontVariationSettings: "'FILL' 1" }}>
              check_circle
            </span>
            <h3 className="text-[20px] font-semibold text-[#086b53] mb-4">Material Uploaded!</h3>
            ...
            {form.type === 'video' && (
              <div className="bg-[#f0eded] border border-[#c4c6d0] rounded-lg p-3 flex items-start gap-2.5">
                ...
                <p className="text-[12px] text-[#44474f] leading-relaxed">
                  <strong className="text-[#03224d]">Direct Upload Limit: 15 MB.</strong> Direct video uploads are restricted to conserve server memory and mobile bandwidth. Please select <span className="font-bold text-[#03224d]">External Link</span> to paste a YouTube, Vimeo, or Google Drive link for video lectures.
                </p>
              </div>
            )}
